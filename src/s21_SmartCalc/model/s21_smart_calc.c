...
int evaluate_expression(char* expression, double* result, double x) {
  ...
  while (*expression_ptr) {
    if (*expression_ptr == 'x') {
      ...
      expression_ptr =
          identify_number_in_expression(expression_ptr, &numbers, &options);
      options.is_operator_last = 0;
      if (options.in_brackets_flag) {
        options.numbers_in_brackets++;
      }
...
char* identify_operator_in_expression(char* expression, Stack* operators,
                                      Stack* numbers, s_options* options) {
...
char* evaluate_operators(char* expression, Stack* numbers, Stack* operators,
                         s_options* options) {
  Node node = get_node_operators(expression);  // текущая нода операции
  if (node.math_operator == LEFT_BRACKET) {
    stack_push(operators, node.value, node.math_operator, node.priority);
    options->in_brackets_flag += 1;
    options->is_operator_last = 1;
  } else if (stack_is_empty(operators)) {  // либо если стек пуст
    stack_push(operators, node.value, node.math_operator, node.priority);
    options->is_operator_last = 1;
  } else if (node.math_operator == RIGHT_BRACKET) {
    right_bracket_handler(numbers, operators, options);
  } else if (node.priority <= operators->top->priority) {
    while (!stack_is_empty(operators) &&
           node.priority <= operators->top->priority) {
...
void right_bracket_handler(Stack* numbers, Stack* operators,
                           s_options* options) {
  double tmp_result = 0;
  ...
  } else if (options->numbers_in_brackets == 0 &&
             stack_top_operator(operators) == LEFT_BRACKET) {
    options->error_code = 1;
  }
  ...
  if (!stack_is_empty(operators) && stack_top_operator(operators) >= SQRT &&
      stack_top_operator(operators) <= LOG) {
  ...
  if (!options->in_brackets_flag) options->numbers_in_brackets = 0;
}
