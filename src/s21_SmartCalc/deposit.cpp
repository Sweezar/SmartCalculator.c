...
{
    static int i = 0;
    if(ui->replenishments_list->item(0) == 0) {
        i = 0;
    ...
    if(periodicity == 0) {
        ui->replenishments_list->insertItem(0, ui->replenishments_combobox->currentText() + " , " + QString::number(month) + " месяц, " + QString::number(replenishment) + " ₽");
    } else {
...
void Deposit::on_withdrawals_push_button_clicked()
{
    ...
    if(periodicity == 0) {
        ui->withdrawals_list->insertItem(0, ui->withdrawals_combobox->currentText() + " , " + QString::number(month) + " месяц, " + QString::number(withdrawal) + " ₽");
    } else {
        ui->withdrawals_list->insertItem(0, ui->withdrawals_combobox->currentText() + " , " + QString::number(withdrawal) + " ₽");
    }
...
void Deposit::on_calculate_push_button_clicked()
{
    ...
    int interest_capitalisation = ui->interest_capitalization_checkbox->isChecked();
    ...
    double interest_charges = deposit_calc(&deposit_amount, posting_period, intererst_rate, interest_capitalisation, capitalisation_period, replenishments, withdrawals);
    ...
    double total_amount = interest_charges + deposit_amount; // доход + начальная ставка
    ui->interes_charges_box->setText(QString::number(interest_charges));
    ui->total_amount_box->setText(QString::number(total_amount));
